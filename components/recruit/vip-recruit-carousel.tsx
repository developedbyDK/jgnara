...
export function VipRecruitCarousel() {
  ...
  React.useEffect(() => {
    async function fetchAds() {
      try {
        const res = await fetch("/api/ads/active?zone=vip-recruit");
        if (res.ok) {
        ...
        <CarouselContent className="-ml-3">
          {ads.map((ad) => (
            ...
            >
              ...
              >
                <div className="relative flex h-[160px] items-center justify-center overflow-hidden rounded-lg shadow-sm transition-transform hover:scale-[1.02]">
                  {/* VIP badge */}
                  <div className="absolute top-3 right-3 z-10 flex items-center gap-1 rounded-full bg-white/20 px-2 py-0.5 backdrop-blur-sm">
                  ...
                  {ad.banner_image_url ? (
                    <img
                      ...
                      alt={ad.ad_products?.name ?? "VIP 구인"}
